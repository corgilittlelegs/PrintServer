...
- **Tier 2 path**: Startup is gated by `profile/VerifiedPrinterProfile(s)` — the parsed IEEE-1284 device ID must positively match a small allowlist (currently just the DeskJet 2300 series) before Tier 2 starts at all; an unmatched legacy printer is refused with a clear message instead of silently getting DeskJet 2300 PCL3-GUI output. `LocalIppServer` answers IPP itself using `PrinterCapabilities.fromProfile(...)` (document formats, media, color modes, quality modes, and resolutions all come from the matched profile, not hardcoded — the `profile` package stays plain Kotlin, `ipp` package does the JIPP translation). `LocalIppServer` parses only the bounded IPP attribute packet off the live HTTP stream (via `DecodedBodyInputStream`) and streams the document payload straight to the spool file, never buffering a whole document in memory; unsupported `document-format` is rejected before any spool file is created. `Print-Job` spools into a single-worker `JobQueue` (deliberately serial — the native Ghostscript/hpcups libraries are not reentrant), which runs it through `RenderingPipeline` (Ghostscript JNI → PPM, `PpmImage` parses it, `hpcups` JNI → PCL3-GUI, wrapped by `NativeCallGuard` which throws loudly if two native encode calls ever overlap) before writing the result to the printer's USB transport. Native call failures are mapped via `HpcupsResult`/`HpcupsNative.fromCode` into setup-failure vs. render-failure categories with distinct, stable codes (see `hpcupsjni.cpp`'s `HPCUPS_ERR_*` constants) rather than one generic error.
- **Legacy USB write serialization**: `usb/LegacyPrinterSession` is a single fair lock shared by `JobQueue`'s rendered-job writes, `Raw9100Relay`'s raw-client writes, and scan-transport closure (`ServerService.closeLegacyTransportForScan`) — all three write to or close the same physical USB transport, and without this lock their operations could interleave and corrupt output. Print jobs wait indefinitely for the lock (`writeExclusive`); raw-9100 clients and scan requests wait a bounded few seconds then are cleanly rejected (`tryWriteExclusive`) rather than hanging or racing. Known gap: `ServerService.stopPipeline()`'s teardown-time transport close does not go through this lock (see the comment above `legacyTransportFor` in `ServerService.kt`) — tracked as a follow-up, not yet fixed.
...
- **Scan pipeline (Spec A)**: `ScanPipeline` drives HP's LEDM scan protocol — real HTTP/1.1 over a raw USB bulk pipe (interface 255/4), the same shape Tier 1 uses for IPP-USB printing — to pull a flatbed scan off the DeskJet 2300-series MFP as a JPEG file, with resolution/color mode configurable per request.
- **eSCL scan server (Spec B)**: `LocalEsclServer` (network-facing sibling of `LocalIppServer`) serves the standard eSCL endpoints, translating between eSCL's XML and `ScanPipeline`'s API; `LedmCapabilities` live-queries the device's real supported resolutions/color modes (mirroring Tier 1's `PrinterQuery` pattern) rather than hardcoding them; advertised over mDNS `_uscan._tcp` via `DiscoveryAdvertiser.advertiseEscl`.

Source layout: `app/src/main/java/dev/jaspreet/printserver/{discovery,http,ipp,jobs,profile,relay,render,scan,service,usb}/` (62 Kotlin files) plus `MainActivity.kt`. `profile/` (`VerifiedPrinterProfile`, `VerifiedPrinterProfiles`) is deliberately plain Kotlin with no JIPP/Android types. Tests: `app/src/test/` (38 JVM unit test files, using `FakePrinterTransport` / `FakeRenderingPipeline`), `app/src/androidTest/` (2 device-only test classes: `LegacyPipelineWiringSmokeTest`, `NativePipelineFixtureTest` — the latter covers single-page, multi-page, and per-quality/color-mode native rendering fixtures — plus bundled `smoke.pdf`/`multipage-smoke.pdf`).
